...
$$IF(VERBOSE)
/*****************************************************************************
** Procedure:  $$PHONE_CLASS_NAME$$::OnSetHookswitch
**
...
** Returns:    void
**
** Description:  This function manages the phoneSetHookswitch processing
**               for this service provider.
**
*****************************************************************************/
$$ENDIF
bool $$PHONE_CLASS_NAME$$::OnSetHookswitch(RTSetHookswitch* pRequest, LPCVOID lpBuff)
{
...
$$ENDIF
	if (pRequest->EnterState(STATE_INITIAL, STATE_WAITING))
	{
		...
		DWORD dwMode = pRequest->GetHookswitchState();		// PHONEHOOKSWITCHMODE_xxx
		// TODO: Adjust the state of the given device. TSP++ has validated that
		// the device's hookswitch can be set (although not necessarily at this moment in
		// time - simply that it is possible) and has validated the mode based on the
		// types given to the wizard.
	}
...
}// $$PHONE_CLASS_NAME$$::OnSetHookswitch
$$ENDIF
$$ENDIF
